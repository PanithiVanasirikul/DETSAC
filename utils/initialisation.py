...
from networks.detsac import DETSAC
...
def get_model(opt):
    ...
    model = DETSAC(input_dim, opt.instances, encoder_blocks=opt.encoder_layers, decoder_blocks=opt.decoder_layers, 
                   nheads=opt.nheads, dim_feedforward=opt.dim_feedforward, batch_norm=True)
    ...
    optimizer = optim.AdamW(model.parameters(), lr=opt.lr, eps=1e-4, weight_decay=1e-4)
    if opt.load is not None and len(opt.load) > 0:
