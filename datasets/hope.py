...
def sampson_distance_from_gt(F, points1, points2):
    Fx1 = points1 @ F.T
    Fx2 = points2 @ F
...
class HopeF:
    ...
    def __init__(self, data_dir, split, cache_data=True, normalize_coords=True, return_images=False, return_gt_models=False):
        ...
        cache_folders = ["/mnt/ssd2/se3_to_image/related_datasets/parsac_cache_folder/hope"]
...
class HopeFDataset(Dataset):
    ...
    def __get_item(self, key):
        ...
        # Not sure if I should augment this
        # if self.augment:
        ...
        if self.return_residual_probs:
            residual_labels = np.zeros((points.shape[0], 4))
            # dist_labels = np.zeros((points.shape[0], 4))
            ...
            for i in range(len(datum['gt'])):
                ...
                dist = sampson_distance_from_gt(F, points1, points2)
                if self.augment:
                    dist = dist*s1*s2
        ...
        return points, points, labels, models, image, imgsize, mask, residual_labels
        # return points, points, labels, models, image, imgsize, mask, residual_labels, dist_labels
